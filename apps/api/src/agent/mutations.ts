    ...
    run: async (input) => {
      ...
      for (const t of unique) {
        try {
          ...
          results.push({ to: t.email, delivered: sent.delivered });
        } catch (e) {
          results.push({ to: t.email, delivered: false, error: (e as Error).message });
        }
      }
      return done("send_email", `Emailed "${subject}" to ${unique.length} recipient(s)`, { key: input.__confirmKey, mailEnabled, results });
    },
  },
];
